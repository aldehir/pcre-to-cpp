...
class CppEmitter:
    ...
    def _generate_literal_match(self, node: LiteralChar, case_insensitive: bool = False):
        ...
        char_comment = self._escape_char(node.char)

        if case_insensitive and node.char.isalpha():
            char_lower = ord(node.char.lower())
            self._emit_block('''
                if (matched && unicode_tolower(_get_cpt(match_pos)) == $char_lower) { // '$char_comment' (case-insensitive)
                    match_pos++;
                } else if (matched) { matched = false; }
            ''', locals())
        else:
            self._emit_block('''
                if (matched && _get_cpt(match_pos) == $char_code) { // '$char_comment'
                    match_pos++;
                } else if (matched) { matched = false; }
            ''', locals())
    ...
    def _generate_special_match(self, node: SpecialChar):
        ...
        self._emit_block('''
            if (matched && _get_cpt(match_pos) == $char_code) { // $char_name
                match_pos++;
            } else if (matched) { matched = false; }
        ''', locals())
