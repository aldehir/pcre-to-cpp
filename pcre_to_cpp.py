...
class CppEmitter:
    ...
    def generate(self, ast: Node) -> str:
        ...
        self._emit("")
        self._emit("// Required helpers (implement these):")
        self._emit("// std::vector<uint32_t> unicode_cpts_from_utf8(const std::string & utf8);")
        self._emit("// unicode_cpt_flags unicode_cpt_flags_from_cpt(uint32_t cpt);")
        self._emit("// uint32_t unicode_tolower(uint32_t cpt);")
        self._emit("// bool unicode_cpt_is_han(uint32_t cpt);")
        self._emit("")

        # Function signature
        self._emit(f"static std::vector<size_t> unicode_regex_split_{self.function_name}(")
        self.indent_level += 1
