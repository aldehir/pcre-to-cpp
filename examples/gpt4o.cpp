                    ...
                    for (size_t i0 = q0_count; i0 > 0; i0--) {
                        ...
                        bt_stack.push_back(match_pos);
                        {
                            while (true) {
                                size_t save_pos = match_pos;
                                matched = true;
                                // Character class: [\p{Lu}\p{Lt}\p{Lm}\p{Lo}\p{M}]
                                if (matched) {
                                    uint32_t cpt_2 = _get_cpt(match_pos);
                                    auto flags_cpt_2 = _get_flags(match_pos);
                                    // Match if any of: \p{Lu} | \p{Lt} | \p{Lm} | \p{Lo} | \p{M}
                                    if (((flags_cpt_2.is_letter && flags_cpt_2.is_uppercase))
                                        || ((flags_cpt_2.is_letter && flags_cpt_2.is_uppercase))
                                        || ((flags_cpt_2.is_letter && !flags_cpt_2.is_uppercase && !flags_cpt_2.is_lowercase))
                                        || ((flags_cpt_2.is_letter && !flags_cpt_2.is_uppercase && !flags_cpt_2.is_lowercase))
                                        || (flags_cpt_2.is_accent_mark)
                                    ) {
                                        match_pos++;
                                    }
                                    else { matched = false; }
                        ...
                        // Try quantifier 1 positions (min_count=0)
                        for (size_t i1 = q1_count; i1 > 0; i1--) {
                            ...
                            bt_stack.push_back(match_pos);
                            {
                                while (true) {
                                    size_t save_pos = match_pos;
                                    matched = true;
                                    // Character class: [\p{Ll}\p{Lm}\p{Lo}\p{M}]
                                    if (matched) {
                                        uint32_t cpt_3 = _get_cpt(match_pos);
                                        auto flags_cpt_3 = _get_flags(match_pos);
                                        // Match if any of: \p{Ll} | \p{Lm} | \p{Lo} | \p{M}
                                        if (((flags_cpt_3.is_letter && flags_cpt_3.is_lowercase))
                                            || ((flags_cpt_3.is_letter && !flags_cpt_3.is_uppercase && !flags_cpt_3.is_lowercase))
                                            || ((flags_cpt_3.is_letter && !flags_cpt_3.is_uppercase && !flags_cpt_3.is_lowercase))
                                            || (flags_cpt_3.is_accent_mark)
                                        ) {
                                            match_pos++;
                                        }
                                        else { matched = false; }
                            ...
                            // Try quantifier 2 positions (min_count=1)
                            for (size_t i2 = q2_count; i2 > 1; i2--) {
                                match_pos = bt_stack[q2_base + i2 - 1];
                                matched = true;
                                // Quantifier 3: (?i:'(s|t|re|ve|m|ll|d))?
                                size_t q3_base = bt_stack.size();
                                bt_stack.push_back(match_pos);
                                {
                                    while (bt_stack.size() - q3_base <= 1) {
                                        size_t save_pos = match_pos;
                                        matched = true;
                                ...
                                size_t q3_count = bt_stack.size() - q3_base;
                                
                                // Try quantifier 3 positions (min_count=0)
                                for (size_t i3 = q3_count; i3 > 0; i3--) {
                                    match_pos = bt_stack[q3_base + i3 - 1];
                                    matched = true;
                                    if (matched) { seq_matched = true; break; }
                                }
                                bt_stack.resize(q3_base);
                        ...
                        bt_stack.push_back(match_pos);
                        {
                            while (true) {
                                ...
                                if (matched) {
                                    ...
                                    // Match if any of: \p{Lu} | \p{Lt} | \p{Lm} | \p{Lo} | \p{M}
                                    if (((flags_cpt_5.is_letter && flags_cpt_5.is_uppercase))
                                        || ((flags_cpt_5.is_letter && flags_cpt_5.is_uppercase))
                                        || ((flags_cpt_5.is_letter && !flags_cpt_5.is_uppercase && !flags_cpt_5.is_lowercase))
                                        || ((flags_cpt_5.is_letter && !flags_cpt_5.is_uppercase && !flags_cpt_5.is_lowercase))
                                        || (flags_cpt_5.is_accent_mark)
                                    ) {
                                        match_pos++;
                                    }
                                    else { matched = false; }
                            ...
                            bt_stack.push_back(match_pos);
                            {
                                while (true) {
                                    ...
                                    if (matched) {
                                        ...
                                        // Match if any of: \p{Ll} | \p{Lm} | \p{Lo} | \p{M}
                                        if (((flags_cpt_6.is_letter && flags_cpt_6.is_lowercase))
                                            || ((flags_cpt_6.is_letter && !flags_cpt_6.is_uppercase && !flags_cpt_6.is_lowercase))
                                            || ((flags_cpt_6.is_letter && !flags_cpt_6.is_uppercase && !flags_cpt_6.is_lowercase))
                                            || (flags_cpt_6.is_accent_mark)
                                        ) {
                                            match_pos++;
                                        }
                                        else { matched = false; }
                        ...
                        bt_stack.push_back(match_pos);
                        {
                            while (true) {
                                ...
                                if (matched) {
                                    ...
                                    bool in_class = (flags_cpt_8.is_whitespace)
                                        || (flags_cpt_8.is_letter)
                                        || (flags_cpt_8.is_number)
                                    ;
                                    if (!in_class && cpt_8 != OUT_OF_RANGE) {
                                        match_pos++;
                                    } else { matched = false; }
                                }
                                if (matched && match_pos > save_pos) {
                ...
                // Sequence with backtracking (1 quantifiers): \s+(?!\S)
                {
                    ...
                    bt_stack.push_back(match_pos);
                    {
                        while (true) {
                            ...
                            matched = true;
                            if (matched) {
                                uint32_t cpt_12 = _get_cpt(match_pos);
                                auto flags_cpt_12 = _get_flags(match_pos);
                                if (flags_cpt_12.is_whitespace) {
                                    match_pos++;
                                } else { matched = false; }
                            }
                            if (matched && match_pos > save_pos) {
                    ...
                    for (size_t i0 = q0_count; i0 > 1; i0--) {
                        ...
                        // Negative lookahead
                        {
                            ...
                            if (matched) {
                                uint32_t cpt_13 = _get_cpt(match_pos);
                                auto flags_cpt_13 = _get_flags(match_pos);
                                if ((!flags_cpt_13.is_whitespace && flags_cpt_13.as_uint())) {
                                    match_pos++;
                                } else { matched = false; }
                            }
            ...
            // Alternative: \s+
            {
                ...
                // One or more
                {
                    ...
                    while (matched) {
                        size_t save_pos = match_pos;
                        if (matched) {
                            uint32_t cpt_14 = _get_cpt(match_pos);
                            auto flags_cpt_14 = _get_flags(match_pos);
                            if (flags_cpt_14.is_whitespace) {
                                match_pos++;
                            } else { matched = false; }
                        }
                        if (!matched || match_pos == save_pos) {
